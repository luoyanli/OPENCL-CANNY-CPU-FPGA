...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.gsn.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.gsn.autotvout_gmem1.dat"
...
parameter gmem1_mem_depth = 32'd 262144;
...
output [64 - 1 : 0] TRAN_gmem1_out_r;
input ready;
input done;
...
reg [gmem1_DATA_BITWIDTH - 1 : 0] gmem1_mem_4 [0: gmem1_mem_depth - 1]; 
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/luoyanl2/ece527_taskpar/fpga_dataflow/gsn_hls/test.prj/solution1/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        4 : WDATA_tmp[j] = gmem1_mem_4[FIFO_AW_req_ADDR_tmp / data_byte_size + counter][j];
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            4 : gmem1_mem_4[FIFO_AW_req_ADDR_tmp / data_byte_size + counter] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
                ...
                while (counter != output_length + 1) begin
                    if (FIFO_WDATA_req_empty != 1) begin 
                            ...
                            if (FIFO_WSTRB_tmp[i] == 1) begin 
                                ...
                                for (j=(i*8); j< (i+1)*8 ; j= j+1) begin
                                    case(FIFO_AW_req_transaction_tmp % mem_page_num)
                                        ...
                                        4 : WDATA_tmp[j] = gmem1_mem_4[FIFO_AW_req_ADDR_tmp / data_byte_size][j];
                                        default: $display("The page_num of AXI write is not valid!");
                                    endcase
                                end
                            end
                        ...
                        case(FIFO_AW_req_transaction_tmp % mem_page_num)
                            ...
                            4 : gmem1_mem_4[FIFO_AW_req_ADDR_tmp / data_byte_size] <= WDATA_tmp;
                            default: $display("The page_num of AXI write is not valid!");
                        endcase
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/luoyanl2/ece527_taskpar/fpga_dataflow/gsn_hls/test.prj/solution1/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            4 : RDATA_tmp <= gmem1_mem_4[FIFO_AR_req_ADDR_tmp / data_byte_size +   counter] ;
                ...
                while (counter != input_length + 1) begin
                    ...
                    if ((write_processing == 1) && (wr_processing_addr == rd_processing_addr) && (wr_processing_trans_num == FIFO_AR_req_transaction_tmp))
                        ...
                        case(FIFO_AR_req_transaction_tmp % mem_page_num)
                            ...
                            4 : RDATA_tmp <= gmem1_mem_4[FIFO_AR_req_ADDR_tmp / data_byte_size ] ;
                            default: $display("The page_num of AXI read is not valid!");
                        endcase
                    RVALID_tmp <= 1;
                    RRESP_tmp <= 0;
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  case(mem_page)
                      ...
                      4 : gmem1_mem_4[i/factor] = mem_tmp;
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem1_mem_depth - gmem1_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  ...
                  4 : tmp_data = gmem1_mem_4[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              ...
              4 : tmp_data = gmem1_mem_4[gmem1_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((gmem1_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  ...
                  3 : $fdisplay(fp,"0x%x",gmem1_mem_3[gmem1_mem_depth/factor][15:0]);
                  4 : $fdisplay(fp,"0x%x",gmem1_mem_4[gmem1_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
