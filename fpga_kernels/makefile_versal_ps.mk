...
LINK_OUTPUT := $(BUILD_DIR)/ced_kernels.link.xsa
...
VPP_PFLAGS := 
CMD_ARGS = $(BUILD_DIR)/ced_kernels.xclbin
SD_CARD := $(PACKAGE_OUT)
dfx_hw := false
...
VPP_FLAGS += --save-temps --jobs 16


EXECUTABLE = ./ced_kernels
...
$(TEMP_DIR)/gau.xo: src/gau.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k gau --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/sobel.xo: src/sobel.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k sobel --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/nms.xo: src/nms.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k nms --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/hyst.xo: src/hyst.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k hyst --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(LINK_OUTPUT): $(TEMP_DIR)/gau.xo $(TEMP_DIR)/sobel.xo $(TEMP_DIR)/nms.xo $(TEMP_DIR)/hyst.xo
	mkdir -p $(BUILD_DIR)
	v++ -l $(VPP_FLAGS) $(VPP_LDFLAGS) -t $(TARGET) --platform $(PLATFORM) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -o $(BUILD_DIR)/ced_kernels.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
...
ifeq ($(dfx_hw), true)
	v++ -p $(VPP_FLAGS) $(LINK_OUTPUT) -t $(TARGET) --platform $(PLATFORM) -o $(BUILD_DIR)/ced_kernels.xclbin 
	v++ -p $(VPP_PFLAGS) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(BUILD_DIR)/ced_kernels.xclbin
else
	v++ -p $(VPP_PFLAGS) $(LINK_OUTPUT) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(EMCONFIG_DIR)/emconfig.json -o $(BUILD_DIR)/ced_kernels.xclbin
endif
