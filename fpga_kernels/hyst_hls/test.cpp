    ...
    for(int yi = 0; yi < ROWS; yi++) {
        for(int xi = 0; xi < COLS; xi++) {
            ...
            if((KERNEL_SIZE < xi && xi < COLS - KERNEL_SIZE) &&
                ...
                out[xi + yi*COLS] = pix_sobel;
                theta[xi + yi*COLS] = grad_sobel;
            }
            else {
                out[xi + yi*COLS] = 0;
                theta[xi + yi*COLS] = grad_sobel;
...
void NonMaxSuppression_tb(uint8_t* data, uint8_t* out, uint8_t* theta) {
    ...
    // #pragma HLS ARRAY_RESHAPE variable=line_buf_val complete dim=1
    // #pragma HLS ARRAY_PARTITION variable=window_buf_val complete dim=0
    // #pragma HLS ARRAY_RESHAPE variable=line_buf_theta complete dim=1
    // #pragma HLS ARRAY_PARTITION variable=window_buf_theta complete dim=0
...
int main() {
    ...
    uint8_t img_2_tb[ROWS*COLS] = {0};
    uint8_t out[ROWS*COLS] = {0};
    uint8_t theta_tb[ROWS*COLS] = {0};
