...
namespace eval debug_profile {
  ...
  proc get_xdp_platform_info {hw_platform_info config_info debug_profile_info} {
    ...
    if {[is_decorated_shell] || [is_soc_shell] || [is_versal_shell]} {
      ocl_util::warning2file $output_dir "INFO: Platform does not require extraction of debug/profile metadata."
      return 0
    }
    ...
    # BEGIN: Pass on using decorations (test purposes only)
    # set_property HDL_ATTRIBUTE.DPA_AXILITE_MASTER true [get_bd_intf_pins "$axilite_intercon/$axilite_master"]
    # if {!$dedicated_master} {
    #   set isDedicatedProp 0
    #   set trace_master [get_bd_intf_pins [lindex $host_masters 0]]
    # } else {
    #   set isDedicatedProp 1
    #   set trace_master [get_bd_intf_pins $aximm_master]
    # }
    # puts "Masters - AXI-Lite: $axilite_intercon/$axilite_master, Trace: $trace_master (dedicated: $dedicated_master)"
    # set_property HDL_ATTRIBUTE.DPA_TRACE_MASTER true $trace_master
    # set_property HDL_ATTRIBUTE.DPA_IS_DEDICATED $isDedicatedProp $trace_master
    # END: Pass on using decorations (test purposes only)
  ...
  proc get_axilite_interconnect { } {
    ...
    if {($intercon == {}) || ($first_cu_clk == {})} {
      puts "--- DEBUG: Unable to find an AXI-Lite interconnect."
      return {}
    }
  ...
  proc is_decorated_shell { } {
    ...
    #puts "--- DEBUG: is_decorated_shell: axiliteMaster = $axiliteMaster, traceOffloadMaster = $traceOffloadMaster"
  ...
  ##################################################################### 
  proc emulation_default_profile_info { profile_info } {
    ...
    puts "--- DEBUG: -----------------------------------------------------------"
    puts "--- DEBUG: Profile info dict for HW emulation:"
    puts "--- DEBUG: $profile_info"
    puts "--- DEBUG: -----------------------------------------------------------"
    return $profile_info
