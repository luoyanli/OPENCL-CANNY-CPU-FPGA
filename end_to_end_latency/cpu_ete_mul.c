...
int main(int argc, char** argv) {
    ...
    cl_command_queue queue_cpu[MAX_QUEUE_NUM];
    for (int i = 0; i < MAX_QUEUE_NUM; i++){
        queue_cpu[i] = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &status);
        assert(status == CL_SUCCESS);
    }
    ...
    FILE *file;
    file = fopen("result_cpumul.csv", "w");
    if (file == NULL) {
    ...
    for (int test_id = 0; test_id  < 1; test_id++){
        for (int task_id = 0; task_id < MAX_TASK_NUM; task_id++) {
            status = clEnqueueWriteBuffer(queue_cpu[task_id],input_buf[task_id] , 0, 0, buffer_size, input_imgs[task_id], 0, NULL, NULL);
            // assert(status == CL_SUCCESS);
            status |= clEnqueueTask(queue_cpu[task_id], gau_kernel[task_id], 0, NULL, &kernel_events[task_id][0]);
            // assert(status == CL_SUCCESS);
            status |= clEnqueueTask(queue_cpu[task_id], sob_kernel[task_id], 0, NULL, &kernel_events[task_id][1]);
            // assert(status == CL_SUCCESS);
            status |= clEnqueueTask(queue_cpu[task_id], nonmax_kernel[task_id], 0, NULL, &kernel_events[task_id][2]);
            // assert(status == CL_SUCCESS);
            status |= clEnqueueTask(queue_cpu[task_id], hyst_kernel[task_id], 0, NULL, &kernel_events[task_id][3]);
            // assert(status == CL_SUCCESS);
            status |= clEnqueueReadBuffer(queue_cpu[task_id], output_buf[task_id], 0, 0, buffer_size, output_imgs[task_id], 0, NULL, NULL);
            assert(status == CL_SUCCESS);
        }
        for (int i = 0; i < MAX_QUEUE_NUM; i++){
