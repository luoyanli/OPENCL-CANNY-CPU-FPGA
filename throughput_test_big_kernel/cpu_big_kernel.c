...
//     cl_kernel gsnh_kernel[MAX_NUM_QUEUE];

//     for (int i = 0; i < MAX_NUM_QUEUE; i++){
//         gsnh_kernel[i] = clCreateKernel(program_cpu, "gsnh", &status);
//     }
...
//         // input_imgs[i] = (uint8_t*)malloc(ROWS*COLS*sizeof(uint8_t));
//         // output_imgs[i] = (uint8_t*)malloc(ROWS*COLS*sizeof(uint8_t));
//     }
...
//     cl_mem in_buf[MAX_NUM_QUEUE];
//     cl_mem out_buf[MAX_NUM_QUEUE];
    
//     for (int i = 0; i < MAX_NUM_QUEUE; i++){
//        in_buf[i] = clCreateBuffer(context_cpu, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, img_buffer_size, input_imgs[i], &status);
//        assert(status == CL_SUCCESS);
//        out_buf[i] = clCreateBuffer(context_cpu, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, img_buffer_size, output_imgs[i], &status);
//          assert(status == CL_SUCCESS);
//     }
...
//     for (int i = 0; i < MAX_NUM_QUEUE; i++ ){
//         status = clSetKernelArg(gsnh_kernel[i], 0, sizeof(cl_mem), (void *)&in_buf[i]);
//         status |= clSetKernelArg(gsnh_kernel[i], 1, sizeof(cl_mem), (void *)&out_buf[i]);
//         assert(status == CL_SUCCESS);
//     }

//     // write results to csv file
//     char filename[50];
//     sprintf(filename, "cpu_%d.csv", ROWS);
...
//             //     status = clEnqueueMigrateMemObjects(queue_cpu[i], 1, &in_buf[i], 0, 0, NULL, NULL);
...
//             for (int i = 0; i < num_queue; i++){
//                 // GSNH kernel
//                 status = clEnqueueTask(queue_cpu[i], gsnh_kernel[i], 0, NULL, NULL);
//                 assert(status == CL_SUCCESS);
//                 // Read the output back to host
//                 // status = clEnqueueReadBuffer(queue_cpu[i], out_buf[i], CL_TRUE, 0, img_buffer_size, output_imgs[i], 0, NULL, NULL);
//                 // assert(status == CL_SUCCESS);
//             }
//             // Finish
//             for (int i = 0; i < num_queue; i++){
...
//             // for (int i = 0; i < num_queue; i++){
//             //     status = clEnqueueMigrateMemObjects(queue_cpu[i], 1, &out_buf[i], CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, NULL);
//             //     clFinish(queue_cpu[i]);
//             // }
...
int main() {
    ...
    printf("Queue size is %d\n", MAX_NUM_QUEUE);
    printf("Total latency: %lu ns\n", total_time / REPETITIONS);
    printf("Latency per Frame: %lu ns\n", total_time /REPETITIONS / MAX_NUM_QUEUE);
    fprintf(file, "%d,%lu,%lu\n", MAX_NUM_QUEUE, total_time / REPETITIONS, total_time /REPETITIONS / MAX_NUM_QUEUE);
