...
int main() {
    ...
    for (int exp = 0; exp <10; exp++){
        ...
        FILE *file;
        char fileName[20];
        sprintf(fileName, "test_%d.csv", exp);
        ...
        for (int TOTAL_TASK = 20; TOTAL_TASK < 61; TOTAL_TASK++){
            fprintf(file, "Total tasks: %d\n", TOTAL_TASK);
            // write title
            fprintf(file, "CPU Tasks Num,FPGA Tasks Num,Total Latency(ns), Lantency per frame (ns)\n");

            cl_kernel gau_kernel_cpu[TOTAL_TASK];
            cl_kernel sob_kernel_cpu[TOTAL_TASK];
            cl_kernel nms_kernel_cpu[TOTAL_TASK];
            cl_kernel hyst_kernel_cpu[TOTAL_TASK];
            ...
            cl_kernel gau_kernel_fpga[TOTAL_TASK];
            cl_kernel sob_kernel_fpga[TOTAL_TASK];
            cl_kernel nms_kernel_fpga[TOTAL_TASK];
            cl_kernel hyst_kernel_fpga[TOTAL_TASK];
            for (int i = 0; i < TOTAL_TASK; i++){
            ...
            uint8_t **input_imgs = (uint8_t**)malloc(TOTAL_TASK * sizeof(uint8_t*));
            uint8_t **input_imgs_cpu = (uint8_t**)malloc(TOTAL_TASK * sizeof(uint8_t*)); 
            uint8_t **output_imgs = (uint8_t**)malloc(TOTAL_TASK * sizeof(uint8_t*));
            uint8_t **output_imgs_cpu = (uint8_t**)malloc(TOTAL_TASK * sizeof(uint8_t*));  
            uint8_t **in_out_imgs = (uint8_t**)malloc(TOTAL_TASK * sizeof(uint8_t*));
            ...
            cl_event kernel_events[TOTAL_TASK][4];
            cl_event migrate_events[TOTAL_TASK][2];
            cl_event write_events[TOTAL_TASK][2];
            cl_event test_event;

            cl_event kernel_events_cpu[TOTAL_TASK][4];
            cl_event migrate_events_cpu[TOTAL_TASK][2];
            cl_event write_events_cpu[TOTAL_TASK][2];
            ...
            int num_queue_cpu = TOTAL_TASK;
            int num_queue_fpga = TOTAL_TASK;
            for (int num_task_cpu = 1; num_task_cpu < TOTAL_TASK; num_task_cpu++){
                int num_task_fpga = TOTAL_TASK - num_task_cpu;
                total_time_cpu = 0;
                ...
                fprintf(file, "%d,%d,%lu,%lu\n", num_task_cpu, num_task_fpga, total_time_cpu / REPETITIONS, total_time_cpu / REPETITIONS/TOTAL_TASK);
            }
