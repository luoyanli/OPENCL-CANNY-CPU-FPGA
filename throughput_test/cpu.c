...
int main() {
    ...
    cl_kernel gau_kernel_cpu[MAX_NUM_QUEUE];
    cl_kernel sob_kernel_cpu[MAX_NUM_QUEUE];
    cl_kernel nms_kernel_cpu[MAX_NUM_QUEUE];
    ...
    printf("[+] input /ouput pages address: %p / %p / %p\n", input_imgs[0], output_imgs[0]);
    ...
    cl_mem in_out_buf1[MAX_NUM_QUEUE];
    cl_mem in_out_buf2[MAX_NUM_QUEUE];
    cl_mem theta_buf[MAX_NUM_QUEUE];
    
    for (int i = 0; i < MAX_NUM_QUEUE; i++){
        in_out_buf1[i] = clCreateBuffer(context_cpu, CL_MEM_READ_WRITE, buffer_size, NULL, &status);
        in_out_buf2[i] = clCreateBuffer(context_cpu, CL_MEM_READ_WRITE, buffer_size, NULL, &status);
        theta_buf[i] = clCreateBuffer(context_cpu, CL_MEM_READ_WRITE, grad_buffer_size, NULL, &status);
    }
    ...
    for (int i = 0; i < MAX_NUM_QUEUE; i++ ){
        status = clSetKernelArg(gau_kernel_cpu[i], 0, sizeof(cl_mem), (void *)&in_out_buf1[i]);
        status |= clSetKernelArg(gau_kernel_cpu[i], 1, sizeof(cl_mem), (void *)&in_out_buf2[i]);
        status = clSetKernelArg(sob_kernel_cpu[i], 0, sizeof(cl_mem), (void *)&in_out_buf2[i]);
        status |= clSetKernelArg(sob_kernel_cpu[i], 1, sizeof(cl_mem), (void *)&theta_buf[i]);
        status = clSetKernelArg(nms_kernel_cpu[i], 0, sizeof(cl_mem), (void *)&theta_buf[i]);
        status |= clSetKernelArg(nms_kernel_cpu[i], 1, sizeof(cl_mem), (void *)&in_out_buf1[i]);
        status = clSetKernelArg(hyst_kernel_cpu[i], 0, sizeof(cl_mem), (void *)&in_out_buf1[i]);
        status |= clSetKernelArg(hyst_kernel_cpu[i], 1, sizeof(cl_mem), (void *)&in_out_buf2[i]);
        assert(status == CL_SUCCESS);
    }
    ...
    for (int num_queue = 1; num_queue < MAX_NUM_QUEUE + 1; num_queue++ ){
        ...
        for (int test_id = 0; test_id < REPETITIONS; test_id++) {
            ...
            for (int i = 0; i < num_queue; i++){
                status = clEnqueueWriteBuffer(queue_cpu[i], in_out_buf1[i], CL_FALSE, 0, buffer_size, input_imgs[i], 0, NULL, NULL);
                // assert(status == CL_SUCCESS);
                // Gaussian kernel
                status = clEnqueueNDRangeKernel(queue_cpu[i], gau_kernel_cpu[i], 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
                assert(status == CL_SUCCESS);

                // Sobel kernel
                status = clEnqueueNDRangeKernel(queue_cpu[i], sob_kernel_cpu[i], 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
                assert(status == CL_SUCCESS);

                // NonMaxSuppression kernel
                status = clEnqueueNDRangeKernel(queue_cpu[i], nms_kernel_cpu[i], 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
                assert(status == CL_SUCCESS);

                // Hystersis kernel
                status = clEnqueueNDRangeKernel(queue_cpu[i], hyst_kernel_cpu[i], 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
                assert(status == CL_SUCCESS);

                // Read the output back to host
                status = clEnqueueReadBuffer(queue_cpu[i], in_out_buf2[i], CL_FALSE, 0, buffer_size, output_imgs[i], 0, NULL, NULL);
                assert(status == CL_SUCCESS);
            }
    ...
    FILE *fp = fopen("cpu.txt", "w");
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < cols; j++) {
            fprintf(fp, "%u ", output_imgs[4][j + i * cols]);
        }
        fprintf(fp, "\n");
    }
    fclose(fp);
    ...
    free(output_imgs);
    for (int i = 0; i < MAX_NUM_QUEUE; i++){
        clReleaseMemObject(in_out_buf1[i]);
        clReleaseMemObject(in_out_buf2[i]);
        clReleaseMemObject(theta_buf[i]);
    }
    clReleaseProgram(program_cpu);
