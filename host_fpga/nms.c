...
int main() {
    ...
    int size = load_file_to_memory("../kernel/nms.xclbin", (char **) &kernelbinary);
    ...
    uint8_t in_out_2[ROWS*COLS] = {0};
    uint8_t *theta = (uint8_t*)malloc(ROWS*COLS*sizeof(uint8_t));
    uint8_t *input_img = (uint8_t*)malloc(ROWS*COLS*sizeof(uint8_t));
    uint8_t *output_img = (uint8_t*)malloc(ROWS*COLS*sizeof(uint8_t));
    posix_memalign((void**)&theta, 4096, ROWS*COLS*sizeof(uint8_t));
    ...
    // Precompute Gaussian and Sobel
    Gaussian_Filter_tb(in_out_1, in_out_2);
    Sobel_Filter_tb(in_out_2, input_img, theta);
    size_t buffer_size = cols * rows * sizeof(char);
    // buffer interact with host
    cl_mem input_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, buffer_size, input_img, &status);
    assert(status == CL_SUCCESS);
    cl_mem output_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, buffer_size, output_img, &status);
    assert(status == CL_SUCCESS);
    cl_mem theta_buf = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, buffer_size, theta, &status);
    assert(status == CL_SUCCESS);
    ...
    status = clSetKernelArg(nonmax_kernel, 0, sizeof(cl_mem), (void *)&input_buf);
    status |= clSetKernelArg(nonmax_kernel, 1, sizeof(cl_mem), (void *)&output_buf);
    status |= clSetKernelArg(nonmax_kernel, 2, sizeof(cl_mem), (void *)&theta_buf);
    assert(status == CL_SUCCESS);
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        // Migrate
        cl_mem mems[3] = {input_buf, output_buf, theta_buf};
        status = clEnqueueMigrateMemObjects(queue, 3, mems, 0, 0, NULL, NULL);
        assert(status == CL_SUCCESS);
        // printf("[+] host->device migrate done\n");

        // Enque kernels
        status = clEnqueueTask(queue, nonmax_kernel, 0, NULL, &kernel_event);
        assert(status == CL_SUCCESS);
