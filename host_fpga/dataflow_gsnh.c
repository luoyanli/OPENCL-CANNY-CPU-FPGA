...
int main(int argc, char** argv) {
    ...
    char *bitstreams[] = {"../kernel/gsnh_nodataflow.xclbin", "../kernel/gsnh.xclbin"};
    ...
    cl_program program = clCreateProgramWithBinary(context, 1, &device_id, 
                            ...
                            NULL, &status);
    assert(status == CL_SUCCESS);
    cl_kernel gsnh_kernel[4];
    gsnh_kernel[0] = clCreateKernel(program, "gsnh", &status);
    gsnh_kernel[1] = clCreateKernel(program, "gsnh", &status);
    gsnh_kernel[2] = clCreateKernel(program, "gsnh", &status);
    gsnh_kernel[3] = clCreateKernel(program, "gsnh", &status);
    assert(status == CL_SUCCESS);
    ...
    for (int i = 0; i < 4; i++)
    {
        status = clSetKernelArg(gsnh_kernel[i], 0, sizeof(cl_mem), (void *)&input_buf[i]);
        status |= clSetKernelArg(gsnh_kernel[i], 1, sizeof(cl_mem), (void *)&output_buf[i]);
        assert(status == CL_SUCCESS);
    }
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        for(int task_id=0; task_id < 4; task_id++) {
            ...
            // Enque kernels
            status = clEnqueueTask(queue, gsnh_kernel[task_id], 1, &migrate_events[task_id], &kernel_events[task_id]);
            assert(status == CL_SUCCESS);
