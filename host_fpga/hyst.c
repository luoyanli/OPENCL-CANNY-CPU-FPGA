...
int main() {
    ...
    int size = load_file_to_memory("../kernel/hyst.xclbin", (char **) &kernelbinary);
    ...
    Sobel_Filter_tb(in_out_2, in_out_3, in_out_1);
    NonMaxSuppression_tb(in_out_3, input_img, in_out_1);
    ...
    status = clSetKernelArg(hyst_kernel, 0, sizeof(cl_mem), (void *)&input_buf);
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        // Enque kernels
        status = clEnqueueTask(queue, hyst_kernel, 0, NULL, &kernel_event);
        assert(status == CL_SUCCESS);
