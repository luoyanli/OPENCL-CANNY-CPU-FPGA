...
int main() {
    ...
    cl_kernel gau_kernel_fpga2 = clCreateKernel(program_fpga, "gau", &status);
    cl_kernel sob_kernel_fpga2 = clCreateKernel(program_fpga, "sobel", &status);
    cl_kernel nms_kernel_fpga2 = clCreateKernel(program_fpga, "nms", &status);
    assert(status == CL_SUCCESS);
    ...
    status = clSetKernelArg(gau_kernel_fpga2, 0, sizeof(cl_mem), (void *)&input_buf2);
    status |= clSetKernelArg(gau_kernel_fpga2, 1, sizeof(cl_mem), (void *)&gau_out_sob_in_buffer2);
    status = clSetKernelArg(sob_kernel_fpga2, 0, sizeof(cl_mem), (void *)&gau_out_sob_in_buffer2);
    status |= clSetKernelArg(sob_kernel_fpga2, 1, sizeof(cl_mem), (void *)&sob_out_nms_in_buffer2);
    status = clSetKernelArg(nms_kernel_fpga2, 0, sizeof(cl_mem), (void *)&sob_out_nms_in_buffer2);
    status |= clSetKernelArg(nms_kernel_fpga2, 1, sizeof(cl_mem), (void *)&fpga_output_buf2);
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        for (int img_id = 0; img_id < TaskNum/2; img_id++){    
            ...
            status |= clEnqueueTask(queue_fpga2, gau_kernel_fpga2, 0, NULL, &kernel_events2[0]);
            status |= clEnqueueTask(queue_fpga, sob_kernel_fpga, 0, NULL, &kernel_events[1]);
            status |= clEnqueueTask(queue_fpga2, sob_kernel_fpga2, 0, NULL, &kernel_events2[1]);
            status |= clEnqueueTask(queue_fpga, nms_kernel_fpga, 0, NULL, &kernel_events[2]);
            status |= clEnqueueTask(queue_fpga2, nms_kernel_fpga2, 0, NULL, &kernel_events2[2]);
            assert(status == CL_SUCCESS);
            // Migrate
    ...
    clReleaseKernel(gau_kernel_fpga2);
    clReleaseKernel(sob_kernel_fpga2);
    clReleaseKernel(nms_kernel_fpga2);
