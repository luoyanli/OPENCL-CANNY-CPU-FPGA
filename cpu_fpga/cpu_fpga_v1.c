...
int main() {
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        for (int img_id = 0; img_id < TaskNum; img_id++){
            ...
            // Execute cpu kernels
            // Write input to CPU
            clWaitForEvents(1, &migrate_events[1]);  // Wait for FPGA to finish
            status = clEnqueueWriteBuffer(queue_cpu, hyst_in_buffer, CL_TRUE, 0, buffer_size, in_out_img, 0, NULL, &write_events[0]);
            status |= clEnqueueTask(queue_cpu, hyst_kernel_cpu, 0, NULL, &kernel_events[3]);
            assert(status == CL_SUCCESS);
            // Read the output back to host
            status = clEnqueueReadBuffer(queue_cpu, cpu_output_buf, CL_TRUE, 0, buffer_size, output_img, 0, NULL, &write_events[1]);
            assert(status == CL_SUCCESS);
            // clWaitForEvents(1, &write_events[1]);  // Wait for CPU to finish
