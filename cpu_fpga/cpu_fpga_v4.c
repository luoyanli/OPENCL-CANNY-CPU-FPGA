...
int main() {
    ...
    int size = load_file_to_memory("../kernel/gsn.xclbin", (char **) &kernelbinary);
    ...
    cl_program program_fpga = clCreateProgramWithBinary(context_fpga, 1, &device_id_fpga, 
                            ...
                            NULL, &status);
    assert(status == CL_SUCCESS);
    cl_kernel gsn_kernel_fpga = clCreateKernel(program_fpga, "gsn", &status);
    cl_kernel gsn_kernel_fpga2 = clCreateKernel(program_fpga, "gsn", &status);
    assert(status == CL_SUCCESS);
    ...
    // Set arguments
    status = clSetKernelArg(gsn_kernel_fpga, 0, sizeof(cl_mem), (void *)&input_buf);
    status |= clSetKernelArg(gsn_kernel_fpga, 1, sizeof(cl_mem), (void *)&fpga_output_buf);
    ...
    assert(status == CL_SUCCESS);
    status = clSetKernelArg(gsn_kernel_fpga2, 0, sizeof(cl_mem), (void *)&input_buf2);
    status |= clSetKernelArg(gsn_kernel_fpga2, 1, sizeof(cl_mem), (void *)&fpga_output_buf2);
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        for (int img_id = 0; img_id < TaskNum/2; img_id++){    
            ...
            assert(status == CL_SUCCESS);
            // Enque kernels
            status = clEnqueueTask(queue_fpga, gsn_kernel_fpga, 0, NULL, &kernel_events[0]);
            status |= clEnqueueTask(queue_fpga2, gsn_kernel_fpga2, 0, NULL, &kernel_events2[0]);
            assert(status == CL_SUCCESS);
            // Migrate
            ...
            status |= clEnqueueTask(queue_cpu, hyst_kernel_cpu, 0, NULL, &kernel_events[1]);
            status |= clEnqueueTask(queue_cpu2, hyst_kernel_cpu2, 0, NULL, &kernel_events2[1]);
            assert(status == CL_SUCCESS);
            ...
            //     assert(status == CL_SUCCESS);
            //     printf("%u: FPGA GSN kernel: %lu ------- %lu\n", 2*img_id, time_start-ref, time_end-ref);
        ...
        if (test_id == REPETITIONS - 1) {
            ...
            FILE *fp = fopen("cpu_fpga_v4.txt", "w");
            for(int i = 0; i < rows; i++) {
                for(int j = 0; j < cols; j++) {
                    fprintf(fp, "%u ", output_img[j + i * cols]);
                    assert(output_img[j + i * cols] == output_img2[j + i * cols]);
                }
                fprintf(fp, "\n");
            }
            fclose(fp);
    ...
    // Release
    clReleaseKernel(gsn_kernel_fpga);
    clReleaseKernel(hyst_kernel_cpu);
    clReleaseKernel(gsn_kernel_fpga2);
