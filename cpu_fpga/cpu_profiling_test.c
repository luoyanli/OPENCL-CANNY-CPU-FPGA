...
int main() {
    ...
    uint8_t *test = (uint8_t*)malloc(sizeof(uint8_t));
    ...
    for (int test_id = 0; test_id < REPETITIONS; test_id++) {
        ...
        for (int img_id = 0; img_id < TaskNum; img_id++){    
            // Migrate
            // printf("Task %d\n", img_id);
            status = clEnqueueMigrateMemObjects(queue_fpga, 1, &input_buf, 0, 0, NULL, NULL);
            assert(status == CL_SUCCESS);
            status = clEnqueueWriteBuffer(queue_cpu, test_buf, CL_FALSE, 0, sizeof(char), test, 0, NULL, &test_event);
            // Enque kernels
            ...
            // Migrate
            status = clEnqueueMigrateMemObjects(queue_fpga, 1, &fpga_output_buf, CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, &migrate_event);
            // Execute cpu kernels
            // Write input to CPU
            clWaitForEvents(1, &migrate_event);
            status = clEnqueueWriteBuffer(queue_cpu, hyst_in_buffer, CL_FALSE, 0, buffer_size, in_out_img, 0, NULL, &write_event[0]);
            clWaitForEvents(1, &write_event[0]);
            clFinish(queue_cpu);
            status |= clEnqueueNDRangeKernel(queue_cpu, hyst_kernel_cpu, 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events[3]);
            assert(status == CL_SUCCESS);
            clWaitForEvents(1, &kernel_events[3]);
            // Read the output back to host
            status = clEnqueueReadBuffer(queue_cpu, cpu_output_buf, CL_FALSE, 0, buffer_size, output_img, 0, NULL, &write_event[1]);
            assert(status == CL_SUCCESS);
            ...
            // Profiling for kernels
                ...
                status |= clGetEventProfilingInfo(migrate_event, CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
                status |= clGetEventProfilingInfo(migrate_event, CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
                printf("%u: FPGA Data write to host: %lu ------- %lu\n", img_id, time_start-ref, time_end-ref);
                status |= clGetEventProfilingInfo(write_event[0], CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
                status |= clGetEventProfilingInfo(write_event[0], CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
                ...
                status |= clGetEventProfilingInfo(write_event[1], CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
                status |= clGetEventProfilingInfo(write_event[1], CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
        ...
        if (test_id == REPETITIONS - 1) {
            ...
            FILE *fp = fopen("cpu_fpga_v1.txt", "w");
            for(int i = 0; i < rows; i++) {
                for(int j = 0; j < cols; j++) {
    ...
    // Print Profiling
    printf("Total time for baseline: %lu ns\n", total_time / REPETITIONS);
