...
int main() {
    ...
    for (int i = 0; i < NUM_KERNEL; i++){
        ...
        gau_kernel_fpga_2[i] = clCreateKernel(program_fpga, "gau", &status);
    }
    assert(status == CL_SUCCESS);
    ...
    cl_mem hyst_out_gau_2_in_buffer[NUM_KERNEL];
    for (int i = 0; i < NUM_KERNEL; i++){
        hyst_out_gau_2_in_buffer[i] = clCreateBuffer(context_fpga, CL_MEM_READ_WRITE, buffer_size, NULL, &status);
        assert(status == CL_SUCCESS);
    }
    printf("[+] all buffer created\n");
    cl_event kernel_events[NUM_KERNEL][5];
    ...
    for (int i = 0; i < NUM_KERNEL; i++ ){
        ...
        status |= clSetKernelArg(hyst_kernel_fpga[i], 1, sizeof(cl_mem), (void *)&hyst_out_gau_2_in_buffer[i]);
        status = clSetKernelArg(gau_kernel_fpga_2[i], 0, sizeof(cl_mem), (void *)&hyst_out_gau_2_in_buffer[i]);
        status |= clSetKernelArg(gau_kernel_fpga_2[i], 1, sizeof(cl_mem), (void *)&fpga_output_buf_2[i]);
        assert(status == CL_SUCCESS);
    }
    /**
    ...
    for (int i = 0; i < NUM_KERNEL; i++){
        ...
        clReleaseMemObject(hyst_out_gau_2_in_buffer[i]);
    }
