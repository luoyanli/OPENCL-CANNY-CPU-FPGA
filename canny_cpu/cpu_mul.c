...
int main(int argc, char** argv) {
    ...
    cl_command_queue queue_1 = clCreateCommandQueueWithProperties(context, device_id, properties, &status);
    assert(status == CL_SUCCESS);
    printf("[+] command queue 1 created\n");
    cl_command_queue queue_2 = clCreateCommandQueueWithProperties(context, device_id, properties, &status);
    assert(status == CL_SUCCESS);
    printf("[+] command queue 2 created\n");
    ...
    for (int test_id = 0; test_id < REPETITIONS * 2; test_id++) {
        ...
        status = clEnqueueWriteBuffer(queue_1, in_out_buf[0], CL_TRUE, 0, img_buffer_size, input_img_1, 0, NULL, NULL);
        assert(status == CL_SUCCESS);
        clFinish(queue_1);
        ...
        // Gaussian kernel
        status = clEnqueueNDRangeKernel(queue_1, gau_kernel[0], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_1[0]);
        assert(status == CL_SUCCESS);
        status = clEnqueueNDRangeKernel(dummy_queue, gau_kernel[1], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_2[0]);
        assert(status == CL_SUCCESS);

        // Sobel kernel
        status = clEnqueueNDRangeKernel(queue_1, sob_kernel[0], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_1[1]);
        assert(status == CL_SUCCESS);
        status = clEnqueueNDRangeKernel(dummy_queue, sob_kernel[1], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_2[1]);
        assert(status == CL_SUCCESS);

        // NonMaxSuppression kernel
        status = clEnqueueNDRangeKernel(queue_1, nonmax_kernel[0], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_1[2]);
        assert(status == CL_SUCCESS);
        status = clEnqueueNDRangeKernel(dummy_queue, nonmax_kernel[1], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_2[2]);
        assert(status == CL_SUCCESS);

        // Hystersis kernel
        status = clEnqueueNDRangeKernel(queue_1, hyst_kernel[0], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_1[3]);
        assert(status == CL_SUCCESS);
        status = clEnqueueNDRangeKernel(dummy_queue, hyst_kernel[1], 1, NULL, &global_work_size, &local_work_size, 0, NULL, &kernel_events_2[3]);
        assert(status == CL_SUCCESS);
        ...
        status = clEnqueueReadBuffer(queue_1, in_out_buf[1], CL_TRUE, 0, img_buffer_size, output_img_1, 0, NULL, NULL);
        assert(status == CL_SUCCESS);
    ...
    // Print Profiling
    printf("Double Queue Canny Edge Detection latency: %lu ns\n", total_time[0] / REPETITIONS);
    printf("Single Queue Canny Edge Detection latency: %lu ns\n", total_time[1] / REPETITIONS);
